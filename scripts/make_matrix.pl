...
my $COMPONENETS 	   = $ARGV[1];



# This is loop 1 for Phage LIST
my $phage_LIST		   = "phage_LIST";
my @PHAGE;
my $counter = 0;
open(F,$phage_LIST) or die "Could not open $phage_LIST\n";
while(my $line = <F>){
	chomp($line);
	$PHAGE[$counter]=$line;
	$counter=$counter+1;
...
close(F);
my %edges;
my $Total_phage=$counter; 
for(my $i = 0; $i<$Total_phage; $i++){ #Phage 1
	for(my $j = 0; $j<$Total_phage; $j++){ #Phage 2
	   	my $phage1=$PHAGE[$i];
	   	my $phage2=$PHAGE[$j];
	   	$edges{$phage1}{$phage2}=0
	}
}

#Loop 2 for Components
open(F,$COMPONENETS) or die "Could not open $COMPONENETS\n";
...
while(my $line = <F>){
	...
	if($clustersize > 1){
	   ...
	   for(my $i=0; $i<$clustersize; $i++){
	       ...
	       my $grepOutput = `grep $geneToRun *.ffn`;
...
for(my $i = 0; $i<$Total_phage; $i++){ #Phage 1
	for(my $j = 0; $j<$Total_phage; $j++){ #Phage 2
	   	my $phage1=$PHAGE[$i];
	   	my $phage2=$PHAGE[$j];
	   	print "$edges{$phage1}{$phage2}, $phage1, $phage2\n";
	}
}
